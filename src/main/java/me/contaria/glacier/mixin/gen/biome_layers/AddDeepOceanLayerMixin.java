...
public abstract class AddDeepOceanLayerMixin implements CrossSamplingLayer {
    ...
    public int sample(LayerSampleContext<?> context, LayerSampler parent, int x, int z) {
        ...
        if (isShallowOcean(center) &&
                isShallowOcean(parent.sample(this.transformX(x + 1), this.transformZ(z))) &&
                isShallowOcean(parent.sample(this.transformX(x + 2), this.transformZ(z + 1))) &&
                isShallowOcean(parent.sample(this.transformX(x + 1), this.transformZ(z + 2))) &&
                isShallowOcean(parent.sample(this.transformX(x), this.transformZ(z + 1)))
        ) {
